...
namespace Aplicacao
{
    public class TwoCaptchaClient
    {
        ...
        public bool SolveRecaptchaV2(string googleKey, string pageUrl, string proxy, ProxyType proxyType)
        { 
            string result; 
                    ...
                    else
                    {
                        if (response.Substring(0, 3) == "OK|")
                        {
                            ...
                            for (int i = 0; i < 24; i++)
                            {
                                ...
                                using (StreamReader answerStream = new StreamReader(answerResp.GetResponseStream(), Encoding.UTF8))
                                {
                                    ...
                                    else
                                    {
                                        if (answerResponse.Substring(0, 3) == "OK|")
                                        {
                                            ...
                                            //object browser;
                                            //browser = System.Diagnostics.Process.Start("chrome.exe", result);
